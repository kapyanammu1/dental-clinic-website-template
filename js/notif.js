$(document).ready(function() {
    const socket = new WebSocket(`ws://ec2-44-204-79-164.compute-1.amazonaws.com/ws/notifications/`);
    ...
    function fetchAppointmentDetails(appointmentId, APIurl) {
        ...
        $.ajax({
            url: APIurl,
            data: {
                'id': appointmentId  // Pass the payment ID to the server
            },
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                updateModalContent(response.appointment_data);
            },
            error: function(xhr, status, error) {

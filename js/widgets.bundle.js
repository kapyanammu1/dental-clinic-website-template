...
var KTCardsWidget17 = {
    init: function() {
        (function() {
            var chartElement = document.getElementById("kt_card_widget_17_chart");
            if (chartElement) {
                var chartOptions = {
                    size: chartElement.getAttribute("data-kt-size") ? parseInt(chartElement.getAttribute("data-kt-size")) : 70,
                    lineWidth: chartElement.getAttribute("data-kt-line") ? parseInt(chartElement.getAttribute("data-kt-line")) : 11,
                    rotate: chartElement.getAttribute("data-kt-rotate") ? parseInt(chartElement.getAttribute("data-kt-rotate")) : 145
                },
                canvasElement = document.createElement("canvas"),
                labelElement = document.createElement("span");

                // Handle old IE versions that require canvas support
                if (typeof G_vmlCanvasManager !== "undefined") {
                    G_vmlCanvasManager.initElement(canvasElement);
                }

                var context = canvasElement.getContext("2d");
                canvasElement.width = canvasElement.height = chartOptions.size;

                // Append label and canvas to the chart element
                chartElement.appendChild(labelElement);
                chartElement.appendChild(canvasElement);

                // Move the canvas context to the center
                context.translate(chartOptions.size / 2, chartOptions.size / 2);
                context.rotate((chartOptions.rotate / 180 - 0.5) * Math.PI);

                var radius = (chartOptions.size - chartOptions.lineWidth) / 2;

                // Draw arc function
                var drawArc = function(strokeColor, lineWidth, progress) {
                    progress = Math.min(Math.max(0, progress || 1), 1); // Limit progress to between 0 and 1
                    context.beginPath();
                    context.arc(0, 0, radius, 0, 2 * Math.PI * progress, false);
                ...
                // Draw background arc
                drawArc("#E4E6EF", chartOptions.lineWidth, 1);

                // Draw primary progress arc
                drawArc(KTUtil.getCssVariableValue("--bs-primary"), chartOptions.lineWidth, 100 / 150);

                // Draw success progress arc
                drawArc(KTUtil.getCssVariableValue("--bs-success"), chartOptions.lineWidth, 0.4);
            }
        })();
    }
};

// Export for module systems if necessary
if (typeof module !== "undefined") {
    module.exports = KTCardsWidget17;
}

// Initialize the widget on DOM content load
